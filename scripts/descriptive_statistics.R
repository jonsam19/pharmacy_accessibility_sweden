...
county_summary_300 <- results_300 |>
  ...
  summarise(
    ...
    population = sum(pop),
    mean_distance_km = weighted.mean(straight_line_distance_km, pop),
    median_distance_km = median(straight_line_distance_km),
    max_distance_km = max(straight_line_distance_km),
...
ggplot(scenario_comparison, aes(x = n_pharmacies)) +
  geom_line(aes(y = pct_within_10km, colour = "10 km")) +
  geom_line(aes(y = pct_within_20km, colour = "20 km")) +
...
ggplot(results_300, aes(x = straight_line_distance_km, weight = pop)) +
  ...
  labs(
    ...
    subtitle = "Population-weighted straight-line distance",
    x = "Straight-line Distance to Nearest Pharmacy (km)",
    y = "Population"
  ) +
  theme_minimal()
...
# 4. Map of accessibility

results_300_sf <- results_300 |>
  st_as_sf(crs = 4326, coords = c("long_grid", "lat_grid"))

ggplot() +
   geom_sf(data = results_300_sf, aes(colour = within_10km_driving)) +
   scale_colour_manual(
